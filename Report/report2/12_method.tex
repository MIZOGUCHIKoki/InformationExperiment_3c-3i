...
\end{align}
ここで，\eqref{equ:直行成分}より，2次元フーリエ変換の直行成分\((u,v)=(0,0)\)の値は，\(F(0,0)\)は，画素値の和である．
\begin{align}
    F(0,0) & = \sum_{x=1}^{N}\sum_{y=1}^{N}f(x,y)\label{equ:直行成分}
\end{align}
...
ここで，\texttt{fft2}後の値は，複素数である．値は，複素数としての値で意味があり，パワースペクトルに対してフィルタを適用してはならない．逆フーリエ変換した複素数値を含むデータを，\texttt{real}関数を用いて実数化する．\scall{2次元フーリエ変換}\sref{src:08_01}\ -\ \sref{src:08_04}．
...
img = ifftshift(ifft2(fft_img)); % 逆2次元フーリエ変換
img = real(img); % 実数化
	\end{lstlisting}
