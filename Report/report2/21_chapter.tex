...
運動残効は，運動方位に選択的な細胞の発火頻度が高くなることで生じる．運動残効は運動を主に処理するMT野（V5）で生じている．
方位残効のメカニズムも運動残効と同じメカニズムである．
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[remember picture]
        \draw[-latex](0,0)--(3.6,0);
        \draw[-latex](0,0)--(0,3)node[midway,left]{\rotatebox{90}{\tiny スパイク発火（活動電位）}};
        \foreach \u \v in {0.6/90,1.2/-45,1.8/0,2.4/45,3.0/90}{
                \draw[thin](\u,-0.1)--(\u,0.1);
                \fill[gray!50,rotate around={\v:($(\u-0.1,-0.3)!0.5!(\u+0.1,-0.8)$)}](\u-0.1,-0.3)rectangle(\u+0.1,-0.8);
            }
        \draw plot[smooth] (\x, {exp(-0.5* \x * \x)/sqrt(2 * pi)});
        % \draw[thick](0.1,0.1)..controls(0.6,0.1)and(1.8,2.8)and(3.0,0.1)..(3.5,0.1);
        % \draw[thick] plot[smooth] coordinates {(0.1,0.1) (0.6,0.1) (1.8,2.8) (3.0,0.1) (3.5,0.1)};
        % \draw[thick](1.8,2.8)to[out=-20,in=180](3.5,0.1);
    \end{tikzpicture}
\end{figure}