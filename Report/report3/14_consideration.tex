...
講義一覧，折れ線グラフ，円グラフのコンテンツを，全ての色覚で内容を判別できるように配色した．
折れ線グラフについては，マークの形状を商品別に変更することで，凡例と照合して商品を区別した．
...
例えば，M錐体がない，または不十分な働きであるD型色覚では，\figref{fig:D型色覚の周波数に対する感度}のように2つの周波数領域で混同が生じる．
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        ...
        \draw(0.0,0)--(0.0,3);
        \draw(0.4,0)--(0.4,3);
        \draw[latex-latex](0,0.6)--(0.4,0.6)node[above right]{\tiny 混同};
        \draw(3.8,0)--(3.8,3);
        \draw(2.8,0)--(2.8,3);
    ...
    \end{tikzpicture}
    \caption{D型色覚の周波数に対する感度\cite{色覚が変化するとどのように色が見えるのか}}
