...
\purpose
\ref{sec:\kadaiad}節より，任意の波はフーリエ変換し，周波数解析できる．
今回の実験では，音声を周波数解析し，振幅スペクトル上でピークのある周波数および振幅データを抽出した後，それらを元に音波を生成する．生成した音波と元となった音波の違いを聴き比べる．
\method
\ref{sec:\kadaicb}で用いた\texttt{sound1.wav}，\texttt{sound2.wav}に対して，周波数解析を行う．
\matlab の\texttt{plot}関数で出力したグラフは，ピークの点をクリックするとその点の値を取り出すことができる．
取り出した点を行列に保存し，その振幅と周波数から純音を生成する．
このとき，時刻\(t\)に対して，周波数\(f\)振幅\(A\)に対して，\(f(t) = A\sin(2\pi ft)\)と表せること，すべての波は正弦波の和で表せることをもとに，生成した純音を合成し，音波を作成する．
\result
\begin{figure}[H]
    \centering
