...
\paragraph{実験内容}この実験では，周波数を\(f_1=440\textrm{Hz}\)，\(f_2=660\textrm{Hz}\)の2種類を用いてそれぞれ正弦波\(y_1\)，\(y_2\)を生成する．生成した\(y_n(n=\{1,2\})\)に対して，\(t\)を横軸に取りグラフを作成し，サンプリング周波数を\(\textrm{\texttt{Fs}}=16\textrm{kHz}\)として再生する．\scall\sref{src:01_01}．\par
...
今回は，サンプリング周波数を\(16\textrm{kHz}\)，平均\(\mu=0\)，標準偏差\(\sigma^2=0.2\)，提示時間\(1\)秒の白色ガウス雑音を生成する．つまり，\(16\textrm{k}\)個の乱数を作成し，それを振幅として音波を形成する．\scall\sref{src:01_05}．
...
今回はサンプリング周波数を\(16\textrm{kHz}\)で音波を形成したが，理論上\(\texttt{Fs}=\infty\)で，\eqref{equ:確率密度関数}\(\mu=0,\sigma^2=0.2\)のグラフを得るであろう．