...
\paragraph{実験内容}この実験では，周波数を\(f_1=440\textrm{Hz}\)，\(f_2=660\textrm{Hz}\)の2種類を用いてそれぞれ正弦波\(y_1\)，\(y_2\)を生成する．生成した\(y_n(n=\{1,2\})\)に対して，\(t\)を横軸に取りグラフを作成し，サンプリング周波数を\(\textrm{\texttt{Fs}}=16000\textrm{Hz}\)として再生する．\scall\sref{src:01_01}．\par
...
\end{leftbar}
つまり，「正規分布の確率関数と等しい確率密度関数を持つ統計的ノイズ」を表す．今回の実験ではガウス雑音を計算機上で構成し，その信号振幅のヒストグラムを作成する．そのヒストグラムがガウス分布（正規分布）に従っているかを確認する．\par
...
今回は，サンプリング周波数\(\textrm{\texttt{Fs}=16\textrm{kHz}}\)，提示時間\(1\)秒の白色ガウス雑音を生成する．つまり，\(16\textrm{k}\)個の乱数を作成し，それを振幅として音波を形成する．\scall\sref{src:01_05}．
...
\consideration
聴音確認による，ガウス雑音の特徴はとらえることができなかった．ヒストグラム\figref{fig:ヒストグラム}を確認すると，平均\(\mu=0\)のガウス分布に従っていると推測する．
今回はサンプリング周波数\(\textrm{\texttt{Fs}}=16\textrm{kHz}\)で音波を形成したが，理論上\(\textrm{\texttt{Fs}}=\infty\)とするとガウス分布を得るであろう．