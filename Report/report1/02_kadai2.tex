...
\method
今回フィルターを適用させる音源は，ドイツ語音階で「C4」から「C5」まで\(0.25\)秒づつ音を提示する．提示する音の順序を\eqref{equ:音の順序}に，各音階の周波数を\tblref{tbl:音階と周波数}に示す．
\begin{align}
...
\end{table}
\paragraph{LPFの作成と適用}
今回はフィルターの行列を作る方法ではなく，周波数テーブルの閉区間\(\big[-375\textrm{Hz},375\textrm{Hz}\big]\)以外の振幅を\(0\)にする方法でフィルターを適用する．（\srcref{src:フィルターを適用する}）\ssrcref{sec:\kadaibb}{src:02_02}
