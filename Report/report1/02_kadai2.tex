...
\end{equation}
これからわかるように，周波数が大きくなるにつれて振幅は小さくなることがわかる．これは\figref{fig:\kadaiba_矩形波の振幅スペクトル}と一致する．ただし，\eqref{equ:矩形波具体例}のように\(1/3, 1/5, \dots\)と変化しないのは\texttt{abs}関数の利用の際に振幅が変化するためと考えられるが（\figref{fig:absを適用した後のデータ}），詳しくは不明である．
